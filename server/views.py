...
from .permissions import IsOwnerOrReadOnlyTrainer 
from django.db import IntegrityError # Import trainer permissions
    ...
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            try:
                ...
                serialized_user = CustomUserSerializer(user)  # Serialize the user data
                return Response({'message': 'Registration successful', 'user': serialized_user.data}, status=status.HTTP_201_CREATED)
            except IntegrityError as e:
                # Check for IntegrityError to handle duplicate email or username
                if 'UNIQUE constraint' in str(e):
                    return Response({'error': 'Email or username already exists.'},)
                else:
...
def user_login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            # Serialize the user data and include it in the response
            user_serializer = CustomUserSerializer(user)  # Use your user serializer
